...
if __name__ == "__main__":
    ...
    dataloader = DroneDataClasificationModule(
        root_dir, train_transforms, val_transforms, batch_size=batch_size, overlap_train=0.35, overlap_val=0.1, num_workers=16,
        prefetch_factor=4,
    ...
    try:
        ...
        if use_checkpoint:
            drone_model = DroneClassificationModel.load_from_checkpoint(
                f'checkpoints/{model_name}/best-drone-classification.ckpt',
                model=model  # Pass required initialization arguments
            )
        else:
